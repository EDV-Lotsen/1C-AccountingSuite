...
Procedure ResultDetailProcessing(Item, Details, StandardProcessing)
	...
	If Details <> Undefined And TypeOf(Details) = Type("DataCompositionDetailsID") Then 
		...
		ReportForm = GetForm("Report.ProfitAndLossByClassDimensions.Form.ReportForm", ParametersStructure,, True);
...
Function ProcessDetailsAtServer(Val ReportRF, Val ResultRF, Val DetailsDataRF, Val UUID_RF, Details)
	...
	//1.
	MainDataCompositionSchema = Reports.ProfitAndLossByClassDimensions.GetTemplate("ProfitAndLossByClassDimensions");
	...
	If DataCompositionSettings <> Undefined Then 
		...
		ReportObject = FormDataToValue(ReportRF, Type("ReportObject.ProfitAndLossByClassDimensions"));
