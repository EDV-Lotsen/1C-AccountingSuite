...
Procedure ResultDetailProcessing(Item, Details, StandardProcessing)
	...
	If Structure <> Undefined And Structure.Item <> Undefined Then
	    ...
	    ReportForm = GetForm("Report.SalesByItemDetail.Form.ReportForm", ParametersStructure,, True);
...
Function ProcessDetailsAtServer(Val ReportRF, Val ResultRF, Val DetailsDataRF, Val UUID_RF)
	...
	ReportObject = FormDataToValue(ReportRF, Type("ReportObject.SalesByItemDetail"));
	ResultRF.Clear();                                                                              
...
Procedure Excel(Command)
	...
	Structure = GeneralFunctions.GetExcelFile("Sales by item summary", Result);
