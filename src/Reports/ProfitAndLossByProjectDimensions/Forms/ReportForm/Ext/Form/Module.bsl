...
Procedure ResultDetailProcessing(Item, Details, StandardProcessing)
	...
	If Details <> Undefined And TypeOf(Details) = Type("DataCompositionDetailsID") Then 
		...
		ReportForm = GetForm("Report.ProfitAndLossByProjectDimensions.Form.ReportForm", ParametersStructure,, True);
...
Function ProcessDetailsAtServer(Val ReportRF, Val ResultRF, Val DetailsDataRF, Val UUID_RF, Details)
	...
	//1.
	MainDataCompositionSchema = Reports.ProfitAndLossByProjectDimensions.GetTemplate("ProfitAndLossByProjectDimensions");
	...
	If DataCompositionSettings <> Undefined Then 
		...
		ReportObject = FormDataToValue(ReportRF, Type("ReportObject.ProfitAndLossByProjectDimensions"));
...
Procedure Excel(Command)
	...
	Structure = GeneralFunctions.GetExcelFile("Profit and loss by project", Result);
