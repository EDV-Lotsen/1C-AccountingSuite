	...
	If WriteMode = DocumentWriteMode.Posting Then
		...
		If (Not ManualAdjustment) Then
			DocumentParameters = New Structure("Ref, PointInTime,   LineItems",
			                                    Ref, PointInTime(), GeneralFunctions.ValueTableCopyFrom(
			                                                        ThisObject, "Product, Location, Layer, Quantity, Amount", Metadata().Attributes));
			Documents.ItemAdjustment.PrepareDataBeforeWrite(AdditionalProperties, DocumentParameters, Cancel);
		EndIf;
...
Procedure Posting(Cancel, Mode)
	...
	Documents.ItemAdjustment.PrepareDataStructuresForPosting(Ref, AdditionalProperties, RegisterRecords);
...
Procedure UndoPosting(Cancel)
	...
	Documents.ItemAdjustment.PrepareDataStructuresForPostingClearing(Ref, AdditionalProperties, RegisterRecords);
