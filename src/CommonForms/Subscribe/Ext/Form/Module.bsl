...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
		If Constants.SubStatus.Get() = ""  Then
				...
				Elsif SubscribeVersion() = 3 Then
						Items.SubStatus.Title = "Your Plan: Bank Accounting Online";
						Items.SubscribeButton.Visible = False;
						Items.TimeLeftLabel.Visible = False;
				Elsif SubscribeVersion() = 4 Then
                        Items.SubStatus.Title = "Your Plan: Free Trial (Accountant Edition)";
						Items.TimeLeftLabel.Visible = False;
				Elsif SubscribeVersion() = 5 Then
						Items.SubStatus.Title = "Your Plan: Uber Ledger";
						Items.SubscribeButton.Visible = False;
						Items.TimeLeftLabel.Visible = False;
                Else
        ...
        If RemainingDays <= 0 AND Constants.SubStatus.Get() = "" Then
                RemainingDays = 0;
				Items.TimeLeftLabel.TextColor = New Color(255,0,0);
...
Procedure SubscribeButton(Command)
	...
	Elsif FreeTrialVersion = 2 Then
		GoToURL("https://pay.accountingsuite.com/monthly?token=jQswFppCrhgGMw8Avoz4&state=" + TenantV());
	Elsif FreeTrialVersion = 4 Then
		GoToURL("https://pay.accountingsuite.com/monthly?token=758pSvQbadlSl2jOg2fF&state=" + TenantV());	
	EndIf;
